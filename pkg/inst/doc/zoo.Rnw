...
from package \pkg{chron} and \code{"dateTime"} in \pkg{timeDate}.
Because not all required methods used to be available for \code{"dateTime"} in older
versions of \pkg{fCalendar}, Section~\ref{sec:fCalendar} has a somewhat outdated example how
to provide such methods so that \code{"zoo"} objects work with \code{"dateTime"} indexes.
