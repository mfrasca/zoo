...
in package \pkg{fSeries} from the \pkg{Rmetrics} suite \citep{zoo:Rmetrics:2008},
...
\code{"timeDate"} from package \pkg{fCalendar} built on top of the \code{"POSIXct"} class
...
\emph{The original version of this section was written when \pkg{fCalendar} and \pkg{zoo}
did not yet include enough methods to attach \code{"timeDate"} indexes to \code{"zoo"}
...
Meanwhile however, both \pkg{zoo} and \pkg{fCalendar}
...
<<yearmon1>>=
zr3 <- zooreg(rnorm(9), start = yearmon(2000), frequency = 12)
zr3
@
