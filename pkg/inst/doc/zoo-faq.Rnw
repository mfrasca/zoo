...
if (any(jan)) Axis(side = 1, at = m[jan], labels = format(m[jan], "%y"), tcl = -0.7)
...
\mysection[10. What other packages use zoo?]{10. What other packages use \pkg{zoo}?}
...
\pkg{fCalendar} & \pkg{Rmetrics} date and time functions: \code{timeDate} usable with \code{zoo} \\ \hline
\pkg{fSeries} & \pkg{Rmetrics} time series functions: \code{as.timeSeries.zoo}, \code{as.zoo.timeSeries} \\ \hline
