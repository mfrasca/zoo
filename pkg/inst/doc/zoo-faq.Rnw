...
\mysection{10. Does zoo work with RMetrics?}

Yes.  \code{timeDate} class objects from the \pkg{fCalendar} package can be used
...
class \code{zoo} and class \code{timeSeries} from the \pkg{fSeries} package.
...
\pkg{fCalendar} & R Metrics date and time functions : timeDate usable with zoo \\ \hline
\pkg{fSeries} & R Metrics time series functions : as.timeSeries.zoo, as.zoo.timeSeries \\ \hline
\pkg{grid} & graphics infrastructure : use with xyplot.zoo \\ \hline
\pkg{its} & irregular time series : as.its.zoo, as.zoo.its \\ \hline
\pkg{lattice} & grid based graphics :  use with xyplot.zoo \\ \hline
\pkg{playwith} & Interactive graphics : works with xylot.zoo \\ \hline
\pkg{YaleToolkit} & Data exploration tools from Yale University : accepts zoo input \\ \hline
\end{tabular}
\end{document}
